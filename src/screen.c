...
static void draw_dot(WScreen *scr, Drawable d, int x, int y, GC gc)
{
	...
	XFillRectangle(dpy, d, gc, x, y + (1 * dot_mult),
				   x + (1 * dot_mult), y + (2 * dot_mult));

	XSetForeground(dpy, gc, scr->white_pixel);
	XFillRectangle(dpy, d, gc, x + (2 * dot_mult), y,
				   x + (3 * dot_mult), y + (2 * dot_mult));
	XFillRectangle(dpy, d, gc, x + (1 * dot_mult), y + (1 * dot_mult),
				   x + (2 * dot_mult), y + (2 * dot_mult));
}
...
static WPixmap *make3Dots(WScreen *scr)
{
	...
	XFillRectangle(dpy, mask, gc2, 4 * dot_mult,
				   wPreferences.icon_size - (6 * dot_mult),
				   3 * dot_mult, 2 * dot_mult);
	XFillRectangle(dpy, mask, gc2, 9 * dot_mult,
				   wPreferences.icon_size - (6 * dot_mult),
				   3 * dot_mult, 2 * dot_mult);
	XFillRectangle(dpy, mask, gc2, 14 * dot_mult,
				   wPreferences.icon_size - (6 * dot_mult),
				   3 * dot_mult, 2 * dot_mult);
