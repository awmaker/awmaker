...
void wMaximizeWindow(WWindow *wwin, int directions, int head)
{
	...
	if (directions & MAX_MAXIMUS) {
		...
		if (directions & MAX_CENTRAL) {
			if (wwin->frame->height > (usableArea.y2 - usableArea.y1)) {
				new_y = usableArea.y1;
			...
			if (wwin->frame->width > (usableArea.x2 - usableArea.x1)) {
				new_x = usableArea.x1;
				...
				unsigned long left, unsigned long right)
{
	...
	if ((int)top < scr->xine_info.count &&
	    (int)bottom < scr->xine_info.count &&
	    (int)left < scr->xine_info.count &&
	    (int)right < scr->xine_info.count) {
	...
	rect1 = scr->xine_info.screens[monitor];

	for (i = 1; i <= 3; i++) {
		monitor = wwin->flags.fullscreen_monitors[i];
		rect2 = scr->xine_info.screens[monitor];
