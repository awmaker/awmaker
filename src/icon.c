...
RImage *icon_get_usable_icon(WWindow *wwin)
{
	...
	/* Use _NET_WM_ICON icon (per-window takes precedence over the shared appicon) */
	if (!image && !WFLAGP(wwin, always_user_icon) && wwin->miniwindow && wwin->miniwindow->net_icon_image)
		image = RRetainImage(wwin->miniwindow->net_icon_image);
