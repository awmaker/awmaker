...
[AC_ARG_VAR([LINGUAS],
    [list of language translations to support (I18N), use 'list' to get the list of supported languages, default: all])dnl
AC_DEFUN([WM_ALL_LANGUAGES],
    [m4_esyscmd([( ls po/ ; ls WPrefs.app/po/ ; ls util/po/ ) | sed -n -e '/po$/{s,\.po,,;p}' | sort -u | tr '\n' ' '])])dnl
...
m4_divert_text([INIT_PREPARE],
    [AS_IF([test "x$LINGUAS" = "x"], [LINGUAS="*"])
...
AS_IF([test "x$LINGUAS" != "x"],
    [wm_save_LIBS="$LIBS"
     ...
     for lang in $LINGUAS; do
         found=0
         wm_missing=""
         m4_foreach([REGION], [wmaker, WPrefs, util, man],
